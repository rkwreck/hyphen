...
export default function AddDealModal({ userId, accessToken, onClose, onAdded }: Props) {
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      if (res.status === 429) { setToast({ message: `Oops! ${data.error}`, type: 'error' }); setParsing(false); return }
  ...
  return (
    <>
      ...
      <div className="bg-white rounded-2xl w-full max-w-md border overflow-hidden" style={{ borderColor: 'var(--p100)' }} onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-4 border-b" style={{ borderColor: 'var(--p100)' }}>
          <h2 className="font-semibold text-base">{stage === 'upload' ? 'Add a new deal' : 'Confirm details'}</h2>
        ...
        <div className="p-6">
          {stage === 'upload' && (
            <>
              ...
              <div
                ...
                style={{ borderColor: dragging ? 'var(--p500)' : 'var(--p200)', background: dragging ? 'var(--p50)' : 'white' }}
              >
                {parsing ? (
                  <>
                    <div className="w-8 h-8 border-2 rounded-full animate-spin" style={{ borderColor: 'var(--p200)', borderTopColor: 'var(--p600)' }} />
                    <p className="text-sm font-medium" style={{ color: 'var(--p600)' }}>Parsing your deal...</p>
                  </>
                ) : (
                  <>
                    <div className="w-14 h-14 rounded-2xl flex items-center justify-center" style={{ background: 'var(--p50)' }}>
                      <svg width="28" height="28" fill="none" stroke="var(--p400)" strokeWidth="1.8" viewBox="0 0 24 24">
                    ...
                    <div className="text-center">
                      <p className="text-sm font-medium" style={{ color: 'var(--p700)' }}>Drop your coupon or gift card here</p>
          ...
          {stage === 'confirm' && parsed && (
            <>
              {imagePreview && <img src={imagePreview} alt="deal" className="w-full rounded-xl mb-4 object-cover" style={{ maxHeight: 140 }} />}
              <p className="text-sm mb-4" style={{ color: '#6b7280' }}>Confirm the details below. Fields outlined in red need your input.</p>
              <div className="flex flex-col gap-3">
                <div>
                  <label className="text-xs font-medium block mb-1" style={{ color: '#374151' }}>Brand / Store name *</label>
                  <input value={parsed.store_name || ''} onChange={e => { setParsed(p => p ? { ...p, store_name: e.target.value } : p); setErrors(er => ({ ...er, store_name: false })) }} placeholder="e.g. Target" style={{ borderColor: errors.store_name ? '#ef4444' : undefined }} />
                ...
                <div>
                  ...
                  <input value={parsed.discount_value || ''} onChange={e => { setParsed(p => p ? { ...p, discount_value: e.target.value } : p); setErrors(er => ({ ...er, discount_value: false })) }} placeholder="e.g. 20% off or $10" style={{ borderColor: errors.discount_value ? '#ef4444' : undefined }} />
                ...
                <div>
                  ...
                  <select value={parsed.category || 'other'} onChange={e => setParsed(p => p ? { ...p, category: e.target.value } : p)} className="w-full" style={{ padding: '9px 12px', border: '1px solid #e5e7eb', borderRadius: 8, fontSize: 14, background: 'white', color: '#1a1a1a', outline: 'none' }}>
              ...
              <div className="flex gap-2 mt-5">
                <button onClick={() => setStage('upload')} className="flex-1 py-2.5 rounded-xl text-sm font-medium border" style={{ borderColor: '#e5e7eb', color: '#6b7280' }}>Re-upload</button>
                <button onClick={handleConfirm} disabled={saving} className="flex-1 py-2.5 rounded-xl text-sm font-medium text-white" style={{ background: 'var(--p600)' }}
